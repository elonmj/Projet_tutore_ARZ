...
Les modèles **microscopiques** se concentrent sur le comportement individuel des véhicules et de leurs conducteurs, simulant les interactions directes telles que le suivi de véhicule (car-following) et les changements de voie . Ils offrent un niveau de détail élevé, permettant d'analyser l'impact des comportements individuels sur le flux global , mais deviennent computationnellement coûteux pour les grands réseaux . Des exemples incluent les modèles stimulus-réponse, les modèles basés sur des points d'action, et les modèles d'automates cellulaires .

À l'opposé, les modèles **macroscopiques** traitent le trafic comme un fluide continu, utilisant des variables agrégées telles que la densité (\(\rho\)), le débit (q), et la vitesse moyenne (v) . Basés sur des équations aux dérivées partielles (EDP) inspirées de la mécanique des fluides , ils sont particulièrement adaptés à l'analyse de réseaux de grande échelle, à la reproduction de phénomènes collectifs (congestion, ondes de choc), et à l'élaboration de stratégies de contrôle du trafic . Leur efficacité computationnelle est un avantage majeur .

Les modèles **mésoscopiques** constituent une approche hybride, combinant des éléments des deux autres [@DrouinSaintLaurent2010]. Ils suivent souvent des véhicules individuels ou des groupes, mais leur comportement est déterminé par des attributs agrégés du flux [@Kalieu2016]. Ils offrent un compromis entre détail et coût computationnel .

**Choix de l'approche macroscopique pour cette thèse :** Cette thèse privilégie l'approche macroscopique pour plusieurs raisons. Son efficacité computationnelle est cruciale pour simuler les réseaux étendus du Bénin . Sa capacité à capturer les phénomènes collectifs comme la congestion est essentielle. De plus, elle est adaptée à l'intégration de l'hétérogénéité du trafic, notamment la forte présence de motos, caractéristique majeure du contexte béninois [@DrouinSaintLaurent2010]. Enfin, face  aux potentielles limitations de données détaillées et aux contraintes de ressources computationnelles dans un pays en développement, l'approche macroscopique représente une option pragmatique .
...
Le modèle **Lighthill-Whitham-Richards (LWR)**, développé indépendamment dans les années 1950, est le pionnier des approches macroscopiques . Il repose sur le principe fondamental de la **conservation du nombre de véhicules** [@DecampsToubon1998], exprimé par l'équation de continuité :
\[
\frac{\partial \rho}{\partial t} + \frac{\partial q}{\partial x} = 0
\]
où \( \rho(x, t) \) est la densité et \( q(x, t) \) est le débit à la position \(x\) et au temps \(t\) .

Une hypothèse clé est l'existence d'une relation d'équilibre statique entre le débit, la densité et la vitesse moyenne \(v\), souvent appelée **diagramme fondamental** : \( q = \rho v \) et \( v = V_e(\rho) \), où \( V_e(\rho) \) est la vitesse d'équilibre, fonction décroissante de la densité [@DecampsToubon1998]. L'équation du modèle devient alors :
\[
...
1.  **Hypothèse d'équilibre instantané :** Il suppose que la vitesse s'ajuste instantanément à \( V_e(\rho) \), ce qui est irréaliste car les conducteurs ont un temps de réaction .
2.  **Incapacité à modéliser les phénomènes hors équilibre :** Il ne peut pas reproduire l'hystérésis (différence de comportement lors de la formation et de la dissipation de la congestion) ni les oscillations *stop-and-go* [@Onimus2003].
3.  **Simplification excessive :** Il ne tient pas compte de l'anticipation, des temps de réaction, ou de l'influence directe des véhicules voisins au-delà de la densité locale .
4.  **Difficulté à gérer l'hétérogénéité :** La relation vitesse-densité unique rend difficile la représentation d'un trafic mixte avec des véhicules aux caractéristiques variées (e.g., motos vs voitures), un point crucial pour le Bénin [@Onimus2003].

Ces lacunes ont motivé le développement de modèles de **second ordre** [@Giorgi2002].

### 2.2.2 Modèles de second ordre

Les modèles macroscopiques de second ordre visent à surmonter les limitations des modèles LWR en introduisant une équation dynamique supplémentaire, généralement pour l'évolution de la vitesse moyenne ou d'une variable liée (quantité de mouvement, énergie) . Cela permet de prendre en compte l'inertie du flux et le temps d'ajustement des vitesses, capturant ainsi les états hors équilibre . Ils peuvent ainsi modéliser des phénomènes comme l'hystérésis  et les ondes de choc (oscillations *stop-and-go*) .

Plusieurs familles de modèles de second ordre existent, comme le modèle de Payne-Whitham (PW) , critiqué pour certains comportements non physiques [@TreiberN/ALecture7], et des modèles généralisés (GSOM) comme METANET . Parmi eux, le modèle **Aw-Rascle-Zhang (ARZ)**, développé indépendamment par Aw & Rascle (2000) et Zhang (2002) , se distingue particulièrement.
...
\]
. Ici, \( v \) est la vitesse moyenne et \( p(\rho) \) est une fonction de "pression" dépendant de la densité, reflétant l'anticipation ou l'hésitation des conducteurs . La quantité \( w = v + p(\rho) \) est un invariant lagrangien, constant le long des trajectoires des véhicules dans un flux homogène .

Certaines formulations incluent un terme de relaxation pour modéliser l'ajustement de la vitesse \( v \) vers une vitesse d'équilibre \( V_e(\rho) \) sur un temps caractéristique \( \tau \) :
\[
...
*   **Anisotropie :** Le modèle respecte le principe selon lequel les conducteurs réagissent principalement aux conditions en aval (devant eux). Les perturbations se propagent vers l'arrière à une vitesse \( \lambda_1 = v - \rho p'(\rho) \), qui est inférieure ou égale à la vitesse des véhicules \( v = \lambda_2 \) .
*   **Capture des phénomènes hors équilibre :** Il modélise les états métastables, l'hystérésis, les transitions congestion/fluide, et les ondes *stop-and-go* .
*   **Pas de vitesses négatives :** Contrairement à certains modèles antérieurs, il évite les vitesses non physiques si \( p'(\rho) \ge 0 \).
*   **Flexibilité :** Le cadre ARZ peut être étendu pour modéliser le trafic multi-classe [@FanWork2015]. Il présente une famille de diagrammes fondamentaux paramétrée par \( w \), offrant une représentation plus riche que le diagramme unique du LWR .
...
*   **Calibration :** La calibration des paramètres, notamment la fonction de pression \( p(\rho) \) et le temps de relaxation \( \tau \), peut être délicate.
*   **Comportements non physiques potentiels :** Des choix inappropriés de \( p(\rho) \) peuvent conduire à des densités maximales multiples ou à des vitesses négatives dans certaines conditions [@FanWork2015]. Des versions modifiées (e.g., GARZ) visent à corriger cela [@FanHertySeibold2013].
...
*   **Diagrammes fondamentaux spécifiques à chaque classe :** Utiliser des relations vitesse-densité \( V_{e,i}(\rho) \) distinctes pour chaque classe \( i \), reflétant leurs différentes vitesses et occupations spatiales .
*   **Coefficients d'équivalence (PCE/PCU) :** Convertir tous les véhicules en un nombre équivalent de voitures particulières pour utiliser un diagramme fondamental unique ou calculer des variables agrégées [@RambhaN/ACE269Lec12].
...
## Bibliographie (citations à convertir en notes de bas de page)

*(Note: Cette liste contient uniquement les références qui doivent être converties en notes de bas de page ou liens directs)*

[@GeminiRevSource1] - General NPTEL course webpage
[@GeminiRevSource4] - ResearchGate Link (Genealogy PDF)
[@GeminiRevSource5] - Wikipedia article
[@GeminiRevSource7] - Wikipedia article
[@GeminiRevSource9] - ResearchGate Link (Large scale design)
[@GeminiRevSource10] - ResearchGate Link (State-of-the-art)
[@GeminiRevSource12] - SIAM Link (Resurrection of 2nd Order)
[@GeminiRevSource13] - General NPTEL course webpage
[@GeminiRevSource14] - Wikipedia article
[@GeminiRevSource21] - Researcher's personal webpage
[@GeminiRevSource26] - Wikipedia article
[@GeminiRevSource34] - Journal Link (Sage - Hysteresis)
[@GeminiRevSource36] - ResearchGate Link (Hysteresis revisited)
[@GeminiRevSource39] - Journal Link (OpenTransportation)
[@GeminiRevSource44] - ResearchGate Link (Multi-class LWR ext)
[@GeminiRevSource45] - ADS Link (Calibration)
[@GeminiRevSource50] - Journal Link (PubsOnLine - Game Theory)
[@GeminiRevSource51] - Journal Link (Banglajol - Num Sim)
[@GeminiRevSource52] - SIAM Link (Resurrection of 2nd Order)
[@GeminiRevSource54] - PDF Link (UCSD - Krstic)
[@GeminiRevSource55] - ResearchGate Link (Developing Aw-Rascle)
[@GeminiRevSource60] - PDF Link (Inria - Costeseque)
[@GeminiRevSource64] - PDF Link (Inria - Costeseque)
[@GeminiRevSource78] - Link to a figure
[@GeminiRevSource81] - YouTube video
[@GeminiRevSource86] - ResearchGate Link (Motorcycle sim)
[@GeminiRevSource87] - PDF Link (EASTS)
[@GeminiRevSource88] - ResearchGate Link (New Approach Mixed)
[@GeminiRevSource92] - ADS Link (VISSIM Motorcycle)
[@GeminiRevSource95] - ResearchGate Link (Motorcycle Impact)
[@GeminiRevSource99] - MDPI Link (Gap-Filling)
[@GeminiRevSource100] - ResearchGate Link (Gap-Filling PDF)
[@GeminiRevSource113] - RePEc Link (Capacity Drop)
...
*   **Équations distinctes par classe :** Formuler un système d'équations ARZ pour chaque classe \( i \), avec des densités \( \rho_i \), des vitesses \( v_i \), et potentiellement des fonctions de pression \( p_i(\rho) \) spécifiques [@FanWork2015; @ColomboMarcellini2020; @Chabchoub2009]. Le système pour N classes serait :
    ...
    \]
    où les fonctions \( p_i \), \( V_{e,i} \), et \( \tau_i \) peuvent dépendre des densités et/ou vitesses de toutes les classes pour modéliser les interactions [@FanWork2015; @ColomboMarcellini2020; @WongWong2002; @BenzoniGavageColombo2003].
...
*   **Paramètres spécifiques par classe :** Attribuer des vitesses de flux libre, des longueurs de véhicule, et des temps de relaxation différents à chaque classe .
...
Le contexte béninois est marqué par une prédominance des motos, en particulier les taxis-motos ("Zémidjans") [@DiFrancescoEtAl2015]. Ces véhicules présentent des comportements spécifiques qui affectent significativement la dynamique du trafic :

*   **Remplissage d'interstices (Gap-filling) :** Capacité des motos à utiliser les espaces entre les véhicules plus grands, leur permettant de progresser même en congestion [@DiFrancescoEtAl2015]. Les modèles microscopiques montrent qu'elles acceptent des intervalles plus petits [@NguyenEtAl2012]. Au niveau macroscopique, cela pourrait être modélisé par une réduction de la densité effective perçue par les motos ou par des termes d'anticipation modifiés .
*   **Entrelacement (Interweaving) / Filtrage / Remontée de file :** Mouvements latéraux continus des motos entre les files de véhicules, surtout à basse vitesse ou à l'arrêt [@DiFrancescoEtAl2015; @TiwariEtAl2007]. Ce comportement optimise l'utilisation de l'espace mais peut perturber le flux des autres véhicules [@DiFrancescoEtAl2015]. La modélisation macroscopique de ce phénomène est complexe et pourrait nécessiter des approches bidimensionnelles ou des modèles à "voies flexibles" [@ColomboMarcelliniRossi2023].
...
*   **Modèles basés sur des analogies physiques :** Utiliser des analogies comme l'effusion de gaz pour le "gap-filling"  ou traiter les motos comme un fluide dans un milieu poreux (les autres véhicules) .
...
**Approches générales :**
*   **Conservation du flux :** Le flux total entrant doit égaler le flux total sortant [@CostesequeSlides].
*   **Règles de distribution :** Utilisation de matrices de distribution ou de coefficients de partage pour déterminer la proportion du flux allant de chaque entrée vers chaque sortie .
...
*   **Gestion de la variable de second ordre :** La variable supplémentaire (e.g., \( w = v + p(\rho) \)) doit également être traitée à la jonction. Des approches consistent à imposer des conditions spécifiques sur cette variable, comme la conservation de \( w \) ou son homogénéisation pour le flux sortant [@HertyEtAl2007].
...
*   **Influence sur les paramètres du modèle :** La qualité de la route affecte principalement la **vitesse de flux libre** \( V_{max} \) et la **capacité** (débit maximal) \( q_{max} \), qui sont des paramètres clés des diagrammes fondamentaux \( V_e(\rho) \) utilisés dans les modèles LWR et ARZ [@JollyEtAl2005]. Des routes dégradées ou des voies étroites entraînent généralement une réduction de ces valeurs [@GuraraEtAl2022].
*   **Intégration dans les modèles :**
    *   **Ajustement des paramètres :** Utiliser des valeurs différentes pour \( V_{max} \), \( q_{max} \), ou d'autres paramètres du diagramme fondamental en fonction du type ou de l'état de la route .
...
*   **Hétérogénéité extrême :** Mélange très diversifié de véhicules motorisés (motos, voitures, camions, bus) et non motorisés (vélos, charrettes), ainsi que des piétons partageant souvent le même espace routier [@Chanut2004].
*   **Prédominance des motos :** Rôle central des deux-roues motorisés (souvent comme taxis) dans la mobilité quotidienne [@DiFrancescoEtAl2015].
*   **Manque de discipline de voie :** Comportements de conduite moins contraints par les voies, avec des mouvements latéraux fréquents et une utilisation opportuniste de l'espace .
...
*   **Limitations de l'infrastructure :** Qualité variable des routes, capacité souvent inadéquate, manque de signalisation ou de marquage au sol [@Chanut2004].
...
*   Des études spécifiques ont été menées dans des contextes similaires (e.g., Inde, Vietnam, autres pays africains) pour adapter les modèles existants ou en développer de nouveaux [@FeldheimLybaert2000; @KnoopDaamen2017].
